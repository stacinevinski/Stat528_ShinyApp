...
# combine our county data with the map data
# map_with_data()
# https://rdrr.io/cran/usmap/man/map_with_data.html
...
# 2019 Top Annual Payroll by Industry by state by year
#Inflation Adj
TopPayAnnState2019 <- df %>% filter(YEAR==2019) %>%
  group_by(FIPS_ST, INDUSTRY) %>%
  summarise(TotalPayAnn = sum(PAYANN_19DOL)) %>%
  filter(TotalPayAnn == max(TotalPayAnn))
TopPayAnnState2019<-rename(TopPayAnnState2019, "fips"="FIPS_ST")

plot_usmap(data = TopPayAnnState2019, values="INDUSTRY") + 
  scale_fill_discrete(name="Industry")+
  labs(title = "2019 Top Industry by Annual Payroll in Each State") + theme(legend.position = "right")

```

```{r}
# 2005 Top Annual Payroll by Industry by state by year
#Inflation Adj
TopPayAnnState2005 <- df %>% filter(YEAR==2005) %>%
  group_by(FIPS_ST, INDUSTRY) %>%
  summarise(TotalPayAnn = sum(PAYANN_19DOL)) %>%
  filter(TotalPayAnn == max(TotalPayAnn))
TopPayAnnState2005<-rename(TopPayAnnState2005, "fips"="FIPS_ST")

plot_usmap(data = TopPayAnnState2005, values="INDUSTRY") + 
  scale_fill_discrete(name="Industry")+
  labs(title = "2005 Top Industry by Annual Payroll in Each State") + theme(legend.position = "right")
```

```{r}
# 2019 Top Annual Payroll by Industry in Iowa by County
#Inflation Adj
TopPayAnnIACty2019 <- df %>% filter(YEAR==2019, STATE=="Iowa") %>%
  group_by(FIPS_CTY, INDUSTRY) %>%
  summarise(TotalPayAnn = sum(PAYANN_19DOL)) %>%
  filter(TotalPayAnn == max(TotalPayAnn))
TopPayAnnIACty2019<-rename(TopPayAnnIACty2019, "fips"="FIPS_CTY")

plot_usmap(data = TopPayAnnIACty2019, values="INDUSTRY", include=c("Iowa")) + 
  scale_fill_discrete(name="Industry")+
  labs(title = "2019 Top Industry by Annual Payroll in Iowa") + theme(legend.position = "right")
```

# Example with Continuous data
This doesn't use our data but a built in dataset.
